...
acertou personagemEmissor personagemReceptor habilidade numeroDado1 numeroDado2 numeroDado3 =
  if (temImunidade personagemReceptor (tipoDeDano habilidade) &&
     selecionaAtributoRelacionado (atributo_relacionado habilidade) personagemEmissor + (min numeroDado1 numeroDado2) >= pontosParaAcerto habilidade)
      then if(((velocidade personagemReceptor) - (min numeroDado1 numeroDado2) > 0) && (velocidade personagemReceptor) - (min numeroDado1 numeroDado2) < numeroDado3) 
      then True
      else False
          ...
          then if(((velocidade personagemReceptor) - (max numeroDado1 numeroDado2) > 0) && (velocidade personagemReceptor) - (max numeroDado1 numeroDado2) < numeroDado3) 
          then True
          else False
